...
const cleanupBeforeExit = async ({ exitCode }) => {
  ...
  if (!cleanupStarted) {
    ...
    try {
      ...
      const cleanupFinished = await Promise.all(
        cleanupWork.map((cleanup) => cleanup())
      );
    } finally {
    ...
    .then(async () => {
      ...
      if (
        ...
        isNonExistingCommandError({
          command: commandWithoutArgs,
          error: result,
        })
      ) {
        log(
          NETLIFYDEVERR,
          `Failed running command: ${command}. Please verify ${chalk.magenta(
            `'${commandWithoutArgs}'`
          )} exists`
        );
      } else {
...
const startFrameworkServer = async function ({ settings }) {
  ...
  log(
    `${NETLIFYDEVLOG} Starting Netlify Dev with ${
      settings.framework || "custom config"
    }`
  );
  ...
  try {
    ...
    if (!open) {
      throw new Error(
        `Timed out waiting for port '${settings.frameworkPort}' to be open`
      );
    }
    ...
    log(
      NETLIFYDEVERR,
      `Netlify Dev could not connect to localhost:${settings.frameworkPort}.`
    );
    log(
      NETLIFYDEVERR,
      `Please make sure your framework server is running on port ${settings.frameworkPort}`
    );
    exit(1);
  }
};
...
const startProxyServer = async ({
  ...
  if (!url) {
    log(
      NETLIFYDEVERR,
      `Unable to start proxy server on port '${settings.port}'`
    );
    exit(1);
  }
...
const startPollingForAPIAuthentication = async function (options) {
  ...
  const helper = async (maybeSiteData) => {
    ...
    const existingAuthlifyTokenId =
      config &&
      config.netlifyGraphConfig &&
      config.netlifyGraphConfig.authlifyTokenId;
...
const dev = async (options, command) => {
  log(`${NETLIFYDEV}`);
  const { api, config, repositoryRoot, site, siteInfo, state } =
    command.netlify;
  ...
  const { addonsUrls, capabilities, siteUrl, timeouts } =
  ...
  const inspectSettings = generateInspectSettings(
    options.edgeInspect,
    options.edgeInspectBrk
  );
  ...
  const liveTunnelUrl = await handleLiveTunnel({
    options,
    site,
    api,
    settings,
  ...
  if (startNetlifyGraphWatcher && options.offline) {
    ...
    const graphEditUrl = getGraphEditUrlBySiteId({
      siteId: site.id,
      oneGraphSessionId,
    });
...
const createDevCommand = (program) => {
  ...
  return program
    ...
    .option("-p ,--port <port>", "port of netlify dev", (value) =>
      Number.parseInt(value)
    )
    .option("--targetPort <port>", "port of target app server", (value) =>
      Number.parseInt(value)
    )
    .option(
      "--framework <name>",
    ...
    .option(
      "-o ,--offline",
      "disables any features that require network access"
    )
    .option("-l, --live", "start a public live session", false)
    .option("--functionsPort <port>", "port of functions server", (value) =>
      Number.parseInt(value)
    )
    .addOption(
      new Option(
    ...
    .addOption(
      new Option(
        "--sessionId [sessionId]",
        "(Graph) connect to cloud session with ID [sessionId]"
      )
    )
    .addOption(
      new Option(
